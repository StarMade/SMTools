    ...
    public static void set(Object to, Object from)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < fromProps.length; i++)
            {
				...
				{
					if (!fromProps[i].getName().equals(toProps[j].getName()))
						continue;
					Class<?> fromType = fromProps[i].getPropertyType();
					Class<?> toType = toProps[j].getPropertyType();
					if ((fromType == null) || (toType == null))
						continue;
					if (!fromType.equals(toType))
						continue;
	                Method read = fromProps[i].getReadMethod();
	                Method write = toProps[j].getWriteMethod();
...
//System.out.println(" Val: "+args[0]);                        
    ...
    public static boolean set(Object to, String key, Object value)
    {
        ...
        try
        {
            ...
            for (int j = 0; j < toProps.length; j++)
            {
                if (!toProps[j].getName().equalsIgnoreCase(key))
                    continue;
                Class<?> toType = toProps[j].getPropertyType();
                if (toType == null)
                    continue;
...
//System.out.println(" Val: "+args[0]);                        
    ...
    public static boolean match(Bean b, String[] cols, String[] vals, boolean isOr, boolean fuzzy)
    {
        ...
        for (int i = 0; i < cols.length; i++)
        {
            ...
            if (v != null)
                if (fuzzy)
                    match = v.toString().indexOf(vals[i]) >= 0;
                else
